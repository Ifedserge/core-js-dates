...
function getTime(date) {
  ...
  const formatedMinuts = minuts < 10 ? `0${minuts}` : minuts;
  const formatedSeconds = seconds < 10 ? `0${seconds}` : seconds;

  return `${formatedHours}:${formatedMinuts}:${formatedSeconds}`;
}
...
function formatDate(date) {
  ...
  const year = dateObj.getUTCFullYear().toString().slice();
  ...
  return `${month}/${day}/${year}, ${hour}:${formattedMinute}:${formattedSecond} ${amPm}`;
}
